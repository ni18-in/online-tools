(function () {
    ...
    btnPasteSample.addEventListener('click', () => {
        ...
        handleTokenChange();
        if (window.showToast) {
            window.showToast("¡Muestra de token cargada!");
        }
    });
    ...
    btnClear.addEventListener('click', () => {
        ...
        handleTokenChange();
        if (window.showToast) {
            window.showToast("Entradas limpiadas.");
        }
    });
    ...
    copyButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (!codeEl || codeEl.textContent === "{}" || codeEl.textContent === "") {
                if (window.showToast) window.showToast("¡Nada que copiar!");
                return;
            }
            const textToCopy = getPlainTextFromJsonOutput(codeEl.textContent);
            navigator.clipboard.writeText(textToCopy).then(() => {
                if (window.showToast) {
                    window.showToast(`¡${targetId === 'headerOutput' ? 'Encabezado' : 'Carga útil'} copiado al portapapeles!`);
                }
            }).catch(err => {
    ...
    function handleTokenChange() {
        ...
        if (!token) {
            ...
            validationStatus.innerHTML = "Ingresa un token JWT para comenzar a decodificar.";
        ...
        if (tokenParts.length !== 3) {
            ...
            validationStatus.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i> Formato de JWT no válido. Un JWT debe constar de Encabezado, Carga útil y Firma separados por puntos.';
        ...
        if (decodedHeaderStr === null || decodedPayloadStr === null) {
            validationStatus.className = "validation-status status-error";
            validationStatus.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i> La decodificación Base64url falló. Asegúrate de que el token esté correctamente codificado.';
            setSignatureStatus("invalid-format");
            return;
        }
        ...
        try {
            ...
            validationStatus.className = "validation-status status-error";
            validationStatus.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i> No se pudieron analizar los segmentos decodificados como JSON.';
            setSignatureStatus("invalid-format");
            return;
        }
        ...
        validationStatus.innerHTML = '<i class="fas fa-check-circle mr-1"></i> La estructura del JWT es válida y se decodificó con éxito.';
    ...
    function parseClaims(payload) {
        ...
        if (payload.hasOwnProperty('exp')) {
            ...
            if (!isNaN(expTime)) {
                ...
                const badgeText = isExpired ? 'Expirado' : 'Activo';
    ...
    function resetSignatureStatus() {
        ...
        signatureStatus.innerHTML = '<i class="fas fa-info-circle mr-1"></i> La verificación de la firma requiere HS256, HS384 o HS512.';
    }

    function setSignatureStatus(type, extra = "") {
        if (type === "invalid-format") {
            signatureStatus.className = "signature-status status-unverified";
            signatureStatus.innerHTML = '<i class="fas fa-exclamation-circle mr-1"></i> No se puede verificar la firma. Formato de token no válido.';
        } else if (type === "unsupported") {
            signatureStatus.className = "signature-status status-unverified";
            signatureStatus.innerHTML = `<i class="fas fa-info-circle mr-1"></i> La verificación de firma no es compatible con el algoritmo <strong>${extra}</strong> (Decodificación únicamente).`;
        } else if (type === "needs-secret") {
            signatureStatus.className = "signature-status status-unverified";
            signatureStatus.innerHTML = '<i class="fas fa-key mr-1"></i> Ingresa una clave secreta arriba para verificar la firma HMAC.';
        } else if (type === "verified") {
            signatureStatus.className = "signature-status status-verified";
            signatureStatus.innerHTML = `<i class="fas fa-check-circle mr-1"></i> ¡Firma verificada! (Firma válida para ${extra})`;
        } else if (type === "failed") {
            signatureStatus.className = "signature-status status-failed";
            signatureStatus.innerHTML = '<i class="fas fa-times-circle mr-1"></i> ¡Firma no válida! La clave secreta no coincide.';
        } else if (type === "error") {
            signatureStatus.className = "signature-status status-failed";
            signatureStatus.innerHTML = '<i class="fas fa-exclamation-triangle mr-1"></i> Error en la verificación de la firma. Verifica el formato del secreto.';
        }
    }
