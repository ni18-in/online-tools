// DOM Element References
        ...
        function saveState() { const stateToSave = deepCopy(currentJsonObject); if (stateToSave === null && currentJsonObject !== null) { showMessage("Error al guardar el estado del historial.", 3000); return; } if (historyStack.length > 0) { const previousStateString = JSON.stringify(historyStack[historyStack.length - 1]); const currentStateString = JSON.stringify(stateToSave); if (previousStateString === currentStateString) return; } historyStack.push(stateToSave); if (historyStack.length > MAX_HISTORY_SIZE) historyStack.shift(); if (redoStack.length > 0) redoStack = []; updateHistoryButtons(); }
        function undo() { if (historyStack.length <= 1) return; const currentState = historyStack.pop(); redoStack.push(currentState); currentJsonObject = deepCopy(historyStack[historyStack.length - 1]); updateJsonInput(); renderVisualization(); updateHistoryButtons(); showMessage("Deshacer completado.", 1500); }
        function redo() { if (redoStack.length === 0) return; const stateToRestore = redoStack.pop(); historyStack.push(stateToRestore); if (historyStack.length > MAX_HISTORY_SIZE) historyStack.shift(); currentJsonObject = deepCopy(stateToRestore); updateJsonInput(); renderVisualization(); updateHistoryButtons(); showMessage("Rehacer completado.", 1500); }
        function parseAndUpdateState(attemptSaveHistory = true) { errorDisplay.textContent = ''; let parsedSuccessfully = false; let tempParsedObject = null; try { const text = jsonInput.value.trim(); if (!text) { tempParsedObject = null; jsonVisualizer.innerHTML = '<p class="text-gray-500 italic">La entrada está vacía.</p>'; } else { tempParsedObject = JSON.parse(text); } currentJsonObject = tempParsedObject; parsedSuccessfully = true; } catch (error) { errorDisplay.textContent = `JSON no válido: ${error.message}`; jsonVisualizer.innerHTML = '<p class="text-red-500 italic font-semibold">No se pudo visualizar debido a errores en el JSON.</p>'; parsedSuccessfully = false; } if (parsedSuccessfully && attemptSaveHistory) { saveState(); } return parsedSuccessfully; }
        function updateJsonInput(format = true) { if (currentJsonObject !== null) { try { jsonInput.value = JSON.stringify(currentJsonObject, null, format ? 2 : undefined); } catch (error) { errorDisplay.textContent = `Error al actualizar la entrada: ${error.message}`; console.error("Error stringifying internal JSON object:", error); } } else { if (historyStack.length > 0 && historyStack[historyStack.length - 1] === null) { jsonInput.value = ''; } } }
        ...
        visualizeBtn.addEventListener('click', () => { if (parseAndUpdateState(true)) { renderVisualization(); showMessage("Visualización actualizada", 1500); } });
        formatBtn.addEventListener('click', () => { if (currentJsonObject !== null) { saveState(); updateJsonInput(true); renderVisualization(); showMessage("JSON formateado", 1500); } else { showMessage("No se puede formatear: JSON no válido.", 2000); } });
        minifyBtn.addEventListener('click', () => { if (currentJsonObject !== null) { saveState(); updateJsonInput(false); renderVisualization(); showMessage("JSON minificado", 1500); } else { showMessage("No se puede minificar: JSON no válido.", 2000); } });
        copyBtn.addEventListener('click', () => { if (!jsonInput.value) { showMessage('¡Nada que copiar!', 1500); return; } navigator.clipboard.writeText(jsonInput.value).then(() => { showMessage('¡JSON copiado!', 1500); }).catch(err => { showMessage('Error al copiar.', 2500); console.error('Clipboard copy failed: ', err); }); });
        clearBtn.addEventListener('click', () => { saveState(); jsonInput.value = ''; currentJsonObject = null; jsonVisualizer.innerHTML = '<p class="text-gray-500 italic">Entrada limpiada.</p>'; errorDisplay.textContent = ''; saveState(); showMessage('Entrada limpiada.', 1500); });
