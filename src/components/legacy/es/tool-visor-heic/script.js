document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processFile(file) {
        ...
        if (!ext.endsWith('.heic') && !ext.endsWith('.heif')) {
            alert('Por favor, selecciona un archivo HEIC o HEIF válido.');
            return;
        }
        ...
        try {
            ...
            console.error(err);
            alert('Error al cargar la imagen HEIC. El archivo podría estar corrupto o no ser compatible.');
            resetViewer();
        }
    }
    ...
    function exportImage() {
        ...
        const downloadName = originalFile.name.replace(/\.heic$/i, '') + `_vista.${downloadExt}`;
