document.addEventListener('DOMContentLoaded', () => {
    ...
    async function processFile(file) {
        ...
        if (!ext.endsWith('.heic') && !ext.endsWith('.heif')) {
            alert('Veuillez sélectionner un fichier HEIC ou HEIF valide.');
            return;
        }
        ...
        try {
            ...
            console.error(err);
            alert('Échec du chargement de l\'image HEIC. Le fichier est peut-être corrompu ou non pris en charge.');
            resetViewer();
        }
    }
    ...
    function displayMetadata() {
        ...
        // Dimensions
        metaDims.textContent = `${currentImage.naturalWidth} x ${currentImage.naturalHeight} px`;
    ...
    function exportImage() {
        ...
        const downloadName = originalFile.name.replace(/\.heic$/i, '') + `_visualise.${downloadExt}`;
