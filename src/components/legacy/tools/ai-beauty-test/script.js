// Wrap the entire script in a DOMContentLoaded listener for robustness
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // --- Utility Functions ---
            function showLoader(loaderId, textElementId = null, initialText = "Loading 0%") {
            ...
            function clearFeatureTimers(featureKey) {
                ...
                if (state.loaderTextId && $('#' + state.loaderTextId).length) {
                    $('#' + state.loaderTextId).text('Loading 0%');
                }
            }
            ...
            function previewImage(inputId, previewImgId) {
                ...
                if (file && file.type.startsWith('image/')) {
                    ...
                    reader.onload = function (e) {
                        ...
                        let baseAlt = $previewElement.attr('alt')?.split(' - ')[0] || "Preview area";
                        $previewElement.attr('alt', `${baseAlt} - Preview of ${file.name}`);
                    }
            ...
            function resetPreview(previewImgId) {
                ...
                const placeholder = `https://placehold.co/${$previewElement.attr('width') || 400}x${$previewElement.attr('height') || 300}/e2e8f0/cbd5e1?text=Upload+Image`;
                $previewElement.attr('src', placeholder);

                let originalAlt = "Preview area for uploaded image";
                // Determine original alt text based on ID
                if (previewImgId === 'scanner-img-preview') originalAlt = "Preview area for uploaded image for beauty scanning";
                else if (previewImgId === 'lookalike-img-preview') originalAlt = "Preview area for uploaded image for celebrity look-alike detection";
                else if (previewImgId === 'analysis-img-preview') originalAlt = "Preview area for uploaded image for face parts analysis";
                else if (previewImgId === 'showdown-img-preview1') originalAlt = "Preview area for first image in beauty showdown";
                else if (previewImgId === 'showdown-img-preview2') originalAlt = "Preview area for second image in beauty showdown";
                else if (previewImgId === 'celebrity-img') originalAlt = "Celebrity Match Placeholder";
            ...
            $('#scanner-upload').on('change', function () {
                ...
                if (file) {
                    ...
                    startProgressiveLoad(featureKey, "Scanning", onCompleteScanner);
                } else {
                    clearFeatureTimers(featureKey);
            ...
            const celebrities = [ // Using slightly more descriptive placeholders
                { name: "Elegance Exemplar", img: "https://placehold.co/160x160/FFC0CB/4a4a4a?text=EE" },
                { name: "Dashing Dynamo", img: "https://placehold.co/160x160/ADD8E6/4a4a4a?text=DD" },
                { name: "Radiant Star", img: "https://placehold.co/160x160/90EE90/4a4a4a?text=RS" },
                { name: "Charming Champ", img: "https://placehold.co/160x160/FFFFE0/4a4a4a?text=CC" },
                { name: "Glamour Icon", img: "https://placehold.co/160x160/E6E6FA/4a4a4a?text=GI" },
                { name: "Heroic Figure", img: "https://placehold.co/160x160/D3D3D3/4a4a4a?text=HF" },
                { name: "Stunning Siren", img: "https://placehold.co/160x160/FFA07A/4a4a4a?text=SS" }
            ];
            $('#lookalike-upload').on('change', function () {
                ...
                if (file) {
                    ...
                    const onCompleteLookalike = () => {
                        ...
                        $('#celebrity-img').attr('src', match.img).attr('alt', `Simulated celebrity match: ${match.name}`);
                        $('#celebrity-confidence').text(confidence);
                    };
                    startProgressiveLoad(featureKey, "Finding Match", onCompleteLookalike);
                } else {
                    clearFeatureTimers(featureKey);
                    if (featureState[featureKey]?.loaderId) hideLoader(featureState[featureKey].loaderId);
                    if (featureState[featureKey]?.resultsId) hideResults(featureState[featureKey].resultsId);
            ...
            const analysisTexts = { // Descriptions remain the same
                eyes: [
                    "Expressive eyes, often indicating a keen observer with a vibrant inner world.",
                    "Deep-set eyes, suggesting a thoughtful, introspective, and perhaps mysterious nature.",
                    "Wide-set eyes, hinting at an open, broad perspective and tolerance for new ideas.",
                    "Bright, clear eyes, reflecting abundant energy, curiosity, and enthusiasm for life.",
                    "Almond-shaped eyes, often seen as balanced, aesthetically pleasing, and potentially indicating artistic sensibility."
                ],
                lips: [
                    "Full lips, often associated with generosity, warmth, and strong nurturing instincts.",
                    "Thin lips, suggesting a more reserved, precise, and perhaps detail-oriented personality.",
                    "A defined cupid's bow, indicating creativity, strong communication skills, and eloquence.",
                    "Balanced, symmetrical lips, reflecting emotional equilibrium, stability, and fairness.",
                    "A naturally upturned smile, conveying approachability, optimism, and a friendly disposition."
                ],
                nose: [
                    "A straight nose bridge, often linked to logic, organization, and a methodical approach.",
                    "A prominent or strong nose, suggesting leadership qualities, ambition, and decisiveness.",
                    "A smaller, refined nose, hinting at sensitivity, attention to detail, and refined tastes.",
                    "A rounded nose tip, associated with a friendly, approachable, and sociable demeanor.",
                    "A slightly upturned nose ('celestial'), sometimes linked to optimism, curiosity, and playfulness."
                ]
            };
            $('#analysis-upload').on('change', function () {
                ...
                const resetAnalysisText = () => {
                    $('#analysis-eyes-text, #analysis-lips-text, #analysis-nose-text').text('Upload an image to see the analysis.');
                }
                ...
                if (file) {
                    ...
                    startProgressiveLoad(featureKey, "Analyzing Features", onCompleteAnalysis);
                } else {
                    clearFeatureTimers(featureKey);
                    if (featureState[featureKey]?.loaderId) hideLoader(featureState[featureKey].loaderId);
                    if (featureState[featureKey]?.resultsId) hideResults(featureState[featureKey].resultsId);
            ...
            function checkShowdownReady() {
                ...
                if (showdownFilesReady.file1 && showdownFilesReady.file2) {
                    const onCompleteShowdown = () => {
                        ...
                        let winnerClass = '';
                        if (score1 > score2) {
                            winnerText = '<i class="fas fa-crown mr-1 sm:mr-2"></i> Image 1 Wins!';
                            winnerClass = 'winner-img1';
                        } else if (score2 > score1) {
                            winnerText = '<i class="fas fa-crown mr-1 sm:mr-2"></i> Image 2 Wins!';
                            winnerClass = 'winner-img2';
                        } else {
                            winnerText = '<i class="fas fa-handshake mr-1 sm:mr-2"></i> It\'s a Tie!';
                    ...
                    startProgressiveLoad(featureKey, "Comparing Scores", onCompleteShowdown);
                } else {
